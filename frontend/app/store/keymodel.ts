...
function handleRenameCurrentTab() {
    ...
    window.dispatchEvent(new CustomEvent("wave:rename-current-tab", { detail: { tabId } }));
}
...
function getDefaultNewBlockDef(): BlockDef {
    const previewBlockDef: BlockDef = {
        meta: {
            view: "preview",
            file: "~",
        },
    ...
    if (blockData?.meta?.connection != null) {
        previewBlockDef.meta.connection = blockData.meta.connection;
    }
    if (blockData?.meta?.file != null) {
        previewBlockDef.meta.file = blockData.meta.file;
    }
    return previewBlockDef;
}
